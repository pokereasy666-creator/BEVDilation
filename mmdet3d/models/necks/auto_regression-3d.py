...
class Mamba_Auto_Regression(nn.Module):
    ...
    def forward(
        ...
        clvl_cruve_template = self.curve_template['curve_template_rank8'].to(coords.device)
        ...
        pos_embed_coords = torch.zeros([coords.shape[0], 9], device=coords.device, dtype=coords.dtype)
        pos_embed_coords[:, 0] = coords[:, 1]
        pos_embed_coords[:, 1:3] = (coords[:, 2:] // 12)
        pos_embed_coords[:, 3:5] = (coords[:, 2:] % 12)
        pos_embed_coords[:, 5:7] = ((coords[:, 2:] + 6) // 12)
        pos_embed_coords[:, 7:9] = ((coords[:, 2:] + 6) % 12)
